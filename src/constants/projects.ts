...
◦ 요청을 얼마나 잘게 쪼개는지가 결과 품질과 비용을 동시에 결정한다는 것을 학습`,
            ...
            {
                title: "주사위 굴리기 횟수가 사라지는 버그",
                badge: "Trouble Shooting",
                problem: `야찌에서 특정 사용자만 주사위 굴리기 횟수(최대 3회)가 한 번에 2회씩 사라지는 문제가 보고됐습니다. 당시 요청 로그가 없어 추적이 어려웠고, 재현 결과 버튼의 disabled가 서버 응답이 도착한 뒤에야 걸려서, 빠른 연타나 마우스 채터링으로 발생한 중복 클릭이 그대로 서버까지 도달해 중복 처리되고 있었습니다.`,
                solution: `◦ UI: 응답을 기다리는 동안 pending 상태로 버튼을 즉시 비활성화
◦ 요청: 액션 함수에 in-flight 가드를 두어 진행 중인 요청의 재발송 차단
◦ 서버: 직전 요청과 700ms 미만 간격의 요청은 무시하는 3중 방어 체계 구축
◦ 방어 수단과 별개로, 다음 추적을 위해 무시된 요청도 로그로 기록`,
                learn: `◦ AI가 구현한 결과는 화면 동작이 아니라 코드로 직접 검증해야 한다는 점을 체감
◦ UI 한 층의 방어로는 부족하며, 요청·서버 계층까지 겹겹이 막아야 안전하다는 것을 학습`,
            },
        ],
    },
    {
        ...
        description: `LLM을 붙이려면 서버, API 키, 토큰 비용, 사용량 관리가 함께 따라옵니다. Chrome이 브라우저에 모델을 내장하면서 이 비용은 사라졌지만, 대신 모델이 작아졌습니다.

하지만 소형 모델은 복잡한 작업에서 품질이 급격히 떨어집니다. 하나의 에이전트에게 전부 시키는 대신 분류·조사·작성·검토를 역할별로 나누면, 각자가 단순한 일만 맡아 결과가 안정됩니다. 온디바이스에서는 멀티 에이전트가 고급 기법이 아니라 품질을 확보하는 기본 조건입니다.

my little agent는 Gemini Nano로 멀티 에이전트 워크플로우를 조립하는 라이브러리입니다. 서버도, 키도, 비용도 없이 사용자의 기기에서 전부 돌아가고, 데이터가 기기 밖으로 나가지 않아 오프라인에서도 동작합니다.
        ...
        contributions: [
            {
                ...
                list: [
                    {
                        title: "",
                        body: `◦ 온디바이스 소형 모델의 품질 한계를 멀티 에이전트 분업으로 극복하는 것을 목표로 방향 설정
◦ Agent, Workflow, Task 모델의 형태를 통일해, 어댑터 없이 중첩 조립되는 API로 정리
◦ Claude를 활용해 TypeScript 기반 환경에서 라이브러리 개발`,
                    },
                ],
            },
            {
                ...
                list: [
                    {
                        title: "",
                        body: `◦ 에이전트를 엮을수록 창이 빠르게 소진되는 문제를 확인하고, 에이전트마다 독립된 창을 부여해 에이전트 N개가 독립된 창 N개로 동작하도록 설계
◦ Chrome Task 모델로 처리 가능한 작업(번역·요약)은 그쪽으로 넘겨 에이전트의 창을 소비하지 않도록 설계
◦ 창에 담을 수 없는 정보는 브라우저 저장소에 두고 필요할 때만 꺼내 쓰도록 설계`,
                    },
                ],
            },
            {
                ...
                list: [
                    {
                        title: "",
                        body: `◦ Chrome Task API를 일반 Agent와 동일한 인터페이스로 편입, 미지원 환경에서 대체 Agent로 자동 우회하는 경로를 설계해 파이프라인이 중단되지 않도록 처리
◦ 계산기, 페이지 읽기, 오늘 날짜 주입 등 소형 모델이 취약한 작업을 도구로 보완`,
                    },
                ],
            },
            {
                ...
                list: [
                    {
                        title: "",
                        body: `◦ 유닛, 타입, 패키징, E2E 4층 테스트 체계를 요구사항으로 정하고 총 98개까지 확보
◦ 사용자 문서(README)와 기여자 문서(AGENTS)를 분리해, 설계 규칙·로드맵·알려진 약점을 별도 관리
◦ npm에 패키지 배포`,
                    },
                ],
            },
